...
def _load_processed_pickle(file_path: Path, require_detailed_data: bool = False) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if require_detailed_data:
            logging.debug(f"[Fusion] Detailed data required, loading from pickle file: {file_path}")
            ...
            if json_file.exists():
                try:
                    ...
                    logging.debug(f"[Fusion] Loaded fusion data from text file (preferred): {json_file}")
                    return result
        ...
        if gene_groups_file.exists() and gene_pairs_file.exists():
            try:
                ...
                logging.debug(f"[Fusion] Loaded fusion data from CSV files (fallback): {analysis_type}")
                return result
        ...
        if file_path.stat().st_size > 0:
            try:
                ...
                if isinstance(data, dict):
                    ...
                    logging.info(f"[Fusion] Generated text files from pickle data for future use: {analysis_type}")
                
                logging.debug(f"[Fusion] Loaded fusion data from pickle file (fallback): {file_path}")
                return data
...
def _generate_summary_files_from_pickle_data(output_dir: Path, data: dict, is_target_panel: bool) -> None:
    ...
    try:
        ...
        logging.info(f"[Fusion] Generated text-based files from pickle data: {analysis_type}")
