...
def add_mgmt_section(launcher: Any, sample_dir: Path) -> None:
    ...
    def _extract_mgmt_specific_sites(bed_path: Path) -> List[Dict[str, Any]]:
        try:
            ...
            if df.shape[1] < 10:
                return []
            cols = ['Chromosome','Start','End','Name','Score','Strand','Start2','End2','RGB','Coverage_Info']
            df = df.iloc[:, :len(cols)]
            df.columns = cols
            ...
            for p1,p2 in cpg_pairs:
                ...
                if not fwd.empty and not rev.empty:
                    cov_f = float(fwd['Coverage'].iloc[0]); cov_r = float(rev['Coverage'].iloc[0])
                    tot = cov_f + cov_r
                    ...
                    label_map = {
                        '129467255/129467256':'Site 1','129467258/129467259':'Site 2','129467262/129467263':'Site 3','129467272/129467273':'Site 4',
                    }
                    pos_key = f"{p1}/{p2}"
                    rows.append({
                        ...
                        'chr': 'chr10',
                        'pos': pos_key,
                        'cov_fwd': int(cov_f), 'cov_rev': int(cov_r), 'cov_total': int(tot),
                        'meth': round(weighted*100.0, 2), 'meth_fwd': round(mf*100.0, 2), 'meth_rev': round(mr*100.0, 2),
    ...
    def _refresh_mgmt() -> None:
        try:
            ...
            try:
                mgmt_results_table.rows = [{'average': f"{average:.2f}", 'pred': f"{pred:.2f}", 'status': status}]
