...
class NanodxAnalysis:
    ...
    def process_parquet_file(self, parquet_path: str, sample_id: str) -> NanodxMetadata:
        ...
        try:
            ...
            predictions, class_labels, n_features = _direct_classification(self.modelfile, test_df)
...
def nanodx_handler(job, work_dir=None):
    ...
    try:
        ...
        job.context.add_metadata('nanodx_processing_steps', nanodx_result.processing_steps)
        ...
        if nanodx_result.error_message:
            ...
            job.context.add_result('nanodx_analysis', {
                'status': 'success',
            ...
            logger.info(f"NanoDX analysis complete for {os.path.basename(parquet_path)}")
            ...
            logger.debug(f"Processing steps: {', '.join(nanodx_result.processing_steps)}")
...
def pannanodx_handler(job, work_dir=None):
    ...
    try:
        ...
        logger.info(f"Starting PanNanoDX analysis for: {os.path.basename(parquet_path)}")
        ...
        job.context.add_metadata('pannanodx_processing_steps', nanodx_result.processing_steps)
        ...
        if nanodx_result.error_message:
            ...
            job.context.add_result('pannanodx_analysis', {
                'status': 'success',
                'sample_id': nanodx_result.sample_id,
                'analysis_time': nanodx_result.analysis_timestamp,
                'model_used': nanodx_result.model_used,
            ...
            logger.info(f"PanNanoDX analysis complete for {os.path.basename(parquet_path)}")
            logger.info(f"Sample ID: {nanodx_result.sample_id}")
            logger.info(f"Model used: {nanodx_result.model_used}")
            logger.info(f"Number of features: {nanodx_result.n_features}")
            logger.debug(f"Processing steps: {', '.join(nanodx_result.processing_steps)}")
