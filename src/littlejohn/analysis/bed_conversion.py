...
try:
	from littlejohn.analysis.temp_utilities import merge_modkit_files
except ImportError:
...
_LOGGER = logging.getLogger("littlejohn.analysis.bed_conversion")
...
class BedConversionAnalysis:
	...
	def _find_cpgs_master_file(self) -> str:
		...
		logger = logging.getLogger("littlejohn.analysis.bed_conversion")
	...
	def _update_state(self, state: str, data: List[str], sample_id: str, file_number: int) -> str:
		...
		if merge_modkit_files is None:
			logger.error("littlejohn.analysis.temp_utilities.merge_modkit_files not available")
			raise ImportError("littlejohn.analysis.temp_utilities.merge_modkit_files is required for parquet creation")
