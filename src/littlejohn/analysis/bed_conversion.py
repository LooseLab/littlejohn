...
class BedConversionAnalysis:
    ...
    def _update_state(
        ...
        if merge_modkit_files is None:
            logger.error(
                "littlejohn.analysis.temp_utilities.merge_modkit_files not available"
            )
            raise ImportError(
                "littlejohn.analysis.temp_utilities.merge_modkit_files is required for parquet creation"
            )
