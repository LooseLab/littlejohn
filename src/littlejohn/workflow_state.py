...
class WorkflowState:
    ...
    def add_job(
        self, job_id: int, job_type: str, filepath: str, sample_id: str, queue_name: str
    ):
    ...
    def get_sample_info(self, sample_id: str) -> Optional[Dict[str, Any]]:
        ...
        with self._lock:
            ...
            sample_jobs = [
                self.jobs[job_id] for job_id in sample_job_ids if job_id in self.jobs
            ]
            ...
            sample_info["completed_jobs"] = len(
                [j for j in sample_jobs if j.status == JobStatus.COMPLETED]
            )
            sample_info["failed_jobs"] = len(
                [j for j in sample_jobs if j.status == JobStatus.FAILED]
            )
            sample_info["running_jobs"] = len(
                [j for j in sample_jobs if j.status == JobStatus.RUNNING]
            )
            sample_info["pending_jobs"] = len(
                [j for j in sample_jobs if j.status == JobStatus.PENDING]
            )
