...
class Job:
    ...
    def _get_display_queue_for_job(self, job_type: str) -> str:
        ...
        if not self.use_separate_analysis_queues and job_type in {
            "mgmt",
            "cnv",
            "target",
            "fusion",
...
class JobProcessor:
    ...
    def _configure_logging(self):
        ...
        try:
            ...
            if self.verbose:
                print(
                    f"JobProcessor for {self.queue_type} initialized with log level: {self.log_level}"
                )
        except Exception as e:
            # Only print warnings if verbose mode is enabled
            if self.verbose:
                print(
                    f"Warning: Could not configure logging for {self.queue_type} actor: {e}"
                )
    ...
    def process_job(self, job_data: bytes) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"status": "error", "error": f"Deserialization error: {e}"}
        ...
        logger.info(
            f"Job {job.job_id} ({job.job_type}) state transition: QUEUED -> PROCESSING"
        )
        ...
        if job.job_type in self.handlers:
            try:
                ...
                logger.info(
                    f"Job {job.job_id} ({job.job_type}) state transition: PROCESSING -> COMPLETED"
                )
                ...
                logger.info(
                    f"Job {job.job_id} ({job.job_type}) state transition: PROCESSING -> FAILED"
                )
                self.failed_jobs += 1
                return {
                    "status": "error",
            ...
            logger.error(f"Job {job.job_id} failed: {error_msg}")
            logger.info(
                f"Job {job.job_id} ({job.job_type}) state transition: QUEUED -> NO_HANDLER"
            )
            self.failed_jobs += 1
            return {
                "status": "error",
    ...
    def get_stats(self) -> Dict[str, int]:
        ...
        return {"processed_jobs": self.processed_jobs, "failed_jobs": self.failed_jobs}
...
class RayWorkflowManager:
    ...
    def __init__(
        ...
        try:
            self.delta_debug_interval = float(
                os.environ.get("LJ_DELTA_DEBUG_INTERVAL", "10")
            )
        except Exception:
        ...
        self.bed_workers_count = (
            bed_workers if isinstance(bed_workers, int) and bed_workers > 0 else 1
        )
        ...
        self._registry_submitted_by_queue: Dict[str, int] = {
            k: 0
            for k in [
                "preprocessing",
                "bed_conversion",
        ...
        self._registry_completed_by_queue: Dict[str, int] = {
            k: 0
            for k in [
                "preprocessing",
                "bed_conversion",
    ...
    def _initialize_processors(self):
        ...
        self.processors["preprocessing"] = [
            JobProcessor.options(
                name=f"JobProcessor-preprocessing-{i+1}", num_cpus=1, max_concurrency=1
            ).remote(
                "preprocessing",
        ...
        self.processors["bed_conversion"] = [
            JobProcessor.options(
                name=f"JobProcessor-bed_conversion-{i+1}", num_cpus=1, max_concurrency=1
            ).remote(
                "bed_conversion",
        ...
        if self.use_separate_analysis_queues:
            ...
            self.processors["mgmt"] = [
                JobProcessor.options(
                    name=f"JobProcessor-mgmt-{i+1}", num_cpus=1, max_concurrency=1
                ).remote("mgmt", self.job_handlers_mgmt, self.log_level, self.verbose)
                for i in range(self.analysis_workers_count)
            ]
            self.processors["cnv"] = [
                JobProcessor.options(
                    name=f"JobProcessor-cnv-{i+1}", num_cpus=1, max_concurrency=1
                ).remote("cnv", self.job_handlers_cnv, self.log_level, self.verbose)
                for i in range(self.analysis_workers_count)
            ]
            self.processors["target"] = [
                JobProcessor.options(
                    name=f"JobProcessor-target-{i+1}", num_cpus=1, max_concurrency=1
                ).remote(
            ...
            self.processors["fusion"] = [
                JobProcessor.options(
                    name=f"JobProcessor-fusion-{i+1}", num_cpus=1, max_concurrency=1
                ).remote(
            ...
            self.processors["analysis"] = [
                JobProcessor.options(
                    name=f"JobProcessor-analysis-1",
                    num_cpus=1,
        ...
        self.processors["classification"] = [
            JobProcessor.options(
                name=f"JobProcessor-classification-1",
                num_cpus=1,
                max_concurrency=analysis_concurrency,
            ).remote(
                "classification",
        ...
        self.processors["slow"] = [
            JobProcessor.options(
                name=f"JobProcessor-slow-1",
                num_cpus=1,
                max_concurrency=analysis_concurrency,
            ).remote("slow", self.job_handlers_slow, self.log_level, self.verbose)
        ]
    ...
    def _on_sample_job_started(self, sample_id: str, job_type: str) -> None:
        ...
        logging.info(
            f"Sample {sample_id} started job {job_type}. Active jobs: {entry['active_jobs']}, Total jobs: {entry['total_jobs']}"
        )
    ...
    def _on_sample_job_finished(
        ...
        with self.progress_lock:
            ...
            entry["last_seen"] = time.time()
        logging.info(
            f"Sample {sample_id} finished job {job_type}. Success: {success}. Active jobs: {entry['active_jobs']}, Completed jobs: {entry['completed_jobs']}, Failed jobs: {entry['failed_jobs']}"
        )
    ...
    def print_priority_status(self) -> None:
        ...
        for queue_type, priority in sorted(
            self.queue_priorities.items(), key=lambda x: x[1], reverse=True
        ):
    ...
    def _can_enqueue_job_for_sample(self, job_type: str, sample_id: str) -> bool:
        ...
        with self.job_tracking_lock:
            running_count = self.running_jobs_by_sample.get(sample_id, {}).get(
                job_type, 0
            )
            pending_count = self.pending_jobs_by_sample.get(sample_id, {}).get(
                job_type, 0
            )
    ...
    def _mark_job_running_for_sample(self, job_type: str, sample_id: str) -> None:
        ...
        with self.job_tracking_lock:
            ...
            if (
                ...
                self.pending_jobs_by_sample[sample_id][job_type] = max(
                    0, self.pending_jobs_by_sample[sample_id][job_type] - 1
                )
    ...
    def _unmark_job_for_sample(self, job_type: str, sample_id: str) -> None:
        ...
        with self.job_tracking_lock:
            if (
                ...
                self.running_jobs_by_sample[sample_id][job_type] = max(
                    0, self.running_jobs_by_sample[sample_id][job_type] - 1
                )
    ...
    def enqueue_jobs(self, jobs: List[Job]) -> None:
        ...
        for job in jobs:
            ...
            if self.verbose:
                print(
                    f"Enqueued job {job.job_id} ({job.job_type}) with priority {priority}"
                )
    ...
    def _submit_job_to_ray(self, job: Job, queue_type: str) -> None:
        ...
        try:
            ...
            try:
                ...
                self.priority_queues.setdefault(prio, []).insert(
                    0,
                    {
                        "job": job,
                        "queue_type": queue_type,
            ...
            if self.verbose:
                print(
                    f"Rescheduled job {job.job_id} ({job.job_type}) after submit error: {submit_exc}"
                )
            return
        ...
        with self.progress_lock:
            sample_id = (
                job.get_sample_id() if hasattr(job, "get_sample_id") else "unknown"
            )
        ...
        if (
            job.job_type in self.deduplicate_job_types
            and sample_id
            and sample_id != "unknown"
        ):
        ...
        if self.verbose:
            print(
                f"Submitted job {job.job_id} ({job.job_type}) to Ray with priority {self.queue_priorities.get(queue_type, 1)}"
            )
    ...
    def process_completed_jobs(self) -> None:
        ...
        for job_id, job_info in active_items:
            ...
            if not self.use_separate_analysis_queues and jt in {
                "mgmt",
                "cnv",
                "target",
                "fusion",
            ...
            with self.progress_lock:
                if job_id in self.active_jobs:
                    if "processing_start_time" in result:
                        self.active_jobs[job_id]["processing_start_time"] = result[
            ...
            if result["status"] == "success":
                ...
                if "triggered_jobs" in result and result["triggered_jobs"]:
                    ...
                    try:
                        ...
                        for jt in result["triggered_job_types"]:
                            q = self._get_queue_type_for_job(jt)
                            triggered_jobs_to_enqueue.append(
                                Job(
                                    job_id=next(_job_id_counter),
                                    job_type=jt,
                                    context=ctx,
                ...
                if triggered_jobs_to_enqueue:
                    ...
                    try:
                        ...
                        fp_name = (
                            os.path.basename(ctx_for_fp.filepath)
                            if ctx_for_fp and hasattr(ctx_for_fp, "filepath")
                            else "unknown"
                        )
                    except Exception:
                        fp_name = "unknown"
                    for tj in triggered_jobs_to_enqueue:
                        try:
                            logging.info(
                                f"File {fp_name}: {result['job_type']} COMPLETED -> {tj.job_type} QUEUED"
                            )
                        except Exception:
                            pass
                    ...
                    if "next_job" in result and result["next_job"]:
                        ...
                        if isinstance(maybe_next, Job):
                            ...
                            try:
                                ...
                                fp_name2 = (
                                    os.path.basename(ctx_for_fp2.filepath)
                                    if ctx_for_fp2 and hasattr(ctx_for_fp2, "filepath")
                                    else "unknown"
                                )
                            except Exception:
                                fp_name2 = "unknown"
                            try:
                                logging.info(
                                    f"File {fp_name2}: {result['job_type']} COMPLETED -> {maybe_next.job_type} QUEUED"
                                )
                            except Exception:
                                pass
                ...
                if "context" in result:
                    ...
                    if sample_id and sample_id != "unknown":
                        self._on_sample_job_finished(
                            sample_id, result["job_type"], True
                        )
                ...
                if ctx is not None:
                    ...
                    if sample_id and sample_id != "unknown":
                        self._on_sample_job_finished(
                            sample_id, result.get("job_type", "unknown"), False
                        )
    ...
    def get_stats(self) -> dict:
        ...
        with self.progress_lock:
            ...
            for job_id, job_info in self.active_jobs.items():
                ...
                duration = (
                    (time.time() - job_info["processing_start_time"])
                    if job_info.get("processing_start_time")
                    else (time.time() - job_info["start_time"])
                )
                sample_id = job_info.get("sample_id") or (
                    os.path.basename(job_info["filepath"]).replace(".bam", "")
                    if job_info["filepath"]
                    else "unknown"
                )
            ...
            completed_by_queue: Dict[str, int] = {
                k: 0
                for k in [
                    "preprocessing",
                    "bed_conversion",
            ...
            submitted_by_queue_reg: Dict[str, int] = (
                self._registry_submitted_by_queue.copy()
            )
            completed_by_queue_reg: Dict[str, int] = (
                self._registry_completed_by_queue.copy()
            )
            ...
            for qname, submitted_q in submitted_by_queue_reg.items():
                ...
                active_q = (
                    len(active_by_queue.get(qname, []))
                    if isinstance(active_by_queue.get(qname, []), list)
                    else 0
                )
            ...
            try:
                overall_total = sum(
                    int(v or 0) for v in submitted_by_queue_reg.values()
                )
            except Exception:
            ...
            try:
                if (
                    self.delta_debug_n
                    and (time.time() - self._last_delta_debug_print_ts)
                    >= self.delta_debug_interval
                ):
                    ...
                    for qname, delta_val in unaccounted_by_queue.items():
                        ...
                        for jid, sub in self._submissions_by_job_id.items():
                            dq = (
                                sub.get("display_queue")
                                or sub.get("queue_type")
                                or "slow"
                            )
                            if dq != qname:
                                continue
                            if (
                                jid in active_jids_dbg
                                or jid in pending_jids_dbg
                                or jid in result_jids_dbg
                            ):
                                continue
                            fp = sub.get("filepath", "")
                            jt = sub.get("job_type", "")
                        ...
                        if examples:
                            try:
                                logging.warning(
                                    f"Δ debug [{qname}]: {delta_val} unaccounted. Examples: {', '.join(examples)}"
                                )
                            except Exception:
                                print(
                                    f"Δ debug [{qname}]: {delta_val} unaccounted. Examples: {', '.join(examples)}",
            ...
            while self.manager.is_running():
                ...
                for queue_type, pbar in progress_bars.items():
                    ...
                    if "completed_by_queue" in stats:
                        completed_q += (
                            stats["completed_by_queue"].get(queue_type, 0) or 0
                        )
                    ...
                    active_in_q = (
                        len(stats.get("active_by_queue", {}).get(queue_type, []))
                        if "active_by_queue" in stats
                        else 0
                    )

                    # Prefer monotonic submitted totals when available for stable progress bars
                    submitted_totals = (
                        stats.get("submitted_by_queue", {})
                        if isinstance(stats.get("submitted_by_queue", {}), dict)
                        else {}
                    )
                    if submitted_totals:
                        total_for_queue = submitted_totals.get(queue_type, 0) or (
                            completed_q + active_in_q + qsize
                        )
                    else:
                    ...
                    if active_in_q > 0:
                        ...
                        failed_q = (
                            stats.get("failed_by_queue", {}).get(queue_type, 0) or 0
                        )
                        skipped_q = (
                            stats.get("skipped_by_queue", {}).get(queue_type, 0) or 0
                        )
                        unacc_q = (
                            stats.get("unaccounted_by_queue", {}).get(queue_type, 0)
                            or 0
                        )
                        pbar.set_postfix_str(
                            f"{sid}({dur:.0f}s) F:{failed_q} S:{skipped_q} Δ:{unacc_q}"
                        )
                    else:
                        failed_q = (
                            stats.get("failed_by_queue", {}).get(queue_type, 0) or 0
                        ...
                        if failed_q or skipped_q or unacc_q:
                            pbar.set_postfix_str(
                                f"F:{failed_q} S:{skipped_q} Δ:{unacc_q}"
                            )
                        else:
                            pbar.set_postfix_str("")
                ...
                if stats.get("queue_sizes"):
                    ...
                    if not stats.get("use_separate_analysis_queues"):
                        ...
                        for qname in ["mgmt", "cnv", "target", "fusion"]:
                            size_est = len(
                                stats.get("active_by_queue", {}).get(qname, [])
                            )
                            if size_est:
