...
def load_modkit_data(parquet_path):
    ...
    if load_minimal_modkit_data is None:
        raise ImportError("load_minimal_modkit_data function not available from robin package")
...
class RandomForestAnalysis:
    ...
    def process_parquet_file(self, parquet_path: str, sample_id: str) -> RandomForestMetadata:
        """Process a single parquet file for Random Forest analysis"""
        logger = logging.getLogger("littlejohn.random_forest")
        
        logger.info(f"Processing parquet file: {parquet_path} for Random Forest analysis")
...
def random_forest_handler(job, work_dir=None):
    ...
    try:
        ...
        logger.info(f"Starting Random Forest analysis for: {os.path.basename(parquet_path)}")
        ...
        job.context.add_metadata('random_forest_processing_steps', result.processing_steps)
        ...
        if result.error_message:
            ...
            job.context.add_result('random_forest_analysis', {
                'status': 'success',
            ...
            logger.info(f"Random Forest analysis complete for {os.path.basename(parquet_path)}")
            ...
            logger.debug(f"Output directory: {os.path.dirname(result.scores_file_path or '')}")
