...
class SturgeonAnalysis:
    ...
    def process_parquet_file(self, parquet_path: str, sample_id: str, work_dir: str) -> SturgeonMetadata:
        ...
        try:
            ...
            self._run_sturgeon_analysis(parquet_path, output_dir, probes_file, current_time)
    ...
    def _run_sturgeon_analysis(self, parquet_path: str, output_dir: str, probes_file: str, current_time: float) -> None:
        ...
        try:
            ...
            sturgeon_bam_background_work(parquet_path, output_dir, str(probes_file), current_time)
...
def sturgeon_handler(job, work_dir=None):
    ...
    try:
        ...
        logger.info(f"Starting Sturgeon analysis for: {os.path.basename(job.context.filepath)}")
        ...
        sturgeon_result = sturgeon_analyzer.process_parquet_file(parquet_path, sample_id, work_dir)
        
        # Store results in job context
        job.context.add_metadata('sturgeon_analysis', sturgeon_result.results)
        job.context.add_metadata('sturgeon_processing_steps', sturgeon_result.processing_steps)
        ...
        if sturgeon_result.error_message:
            ...
            job.context.add_result('sturgeon_analysis', {
                'status': 'success',
            ...
            logger.info(f"Sturgeon analysis complete for {os.path.basename(job.context.filepath)}")
            logger.info(f"Sample ID: {sturgeon_result.sample_id}")
            logger.info(f"Output directory: {sturgeon_result.output_dir}")
            logger.debug(f"Processing steps: {', '.join(sturgeon_result.processing_steps)}")
